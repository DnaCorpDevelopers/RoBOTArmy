names = ['joe', 'barista', 'barman', 'ehi', 'yo', 'hey', 'hei', 'jo']

titles = ['', ' Messere', ' Sir', ' Comandante', 'Mozzo']

greetings = [
    'Signore e signori, il bar è ora aperto!',
    'Gentili clienti, è arrivata nuova merce!',
    'Gentaglia! È qui la festa!'
]

thankAnswer = [
    'grazie',
    'gentile',
    'mancia',
    'resto'
]

drinkList = [
    'drinks',
    'servi'
    'menu',
    'carta'
]

drinkListIncipit = [
    'Abbiamo: ',
    'A lei: ',
    'Ecco: ',
    'Solo il meglio per voi:'
]

everybodyRequest = [
    'tutti',
    'ciurma'
]

sendToEveryone = [
    '@everyone il prossimo giro di {2} lo offre {1} {0}!',
    'Sentito ciurma? {1} {0} offre a @everyone il prossimo giro di {2}!',
    'Informiamo la gentile clientela che {1} {0} offre a @everyone {2}!',
    '@everyone , il prossimo TS di {2} è offerto da {1} {0}!'
]

orders = [
    'Desidera?',
    'Si?',
    'Mi dica.',
    'Vuole ordinare?',
    'Mi dica messer {}.',
    'Comandante {}, sono a sua disposizione.',
    'Ah, {},... mi dica, ma in fretta.',
    'Come posso essere utile?',
    'Di nuovo?',
    'Sono a sua disposizione.',
    'Eccomi!',
    'Il solito?',
    'Bocca asciutta?'
]

thanks = [
    'Prego!',
    'Di nulla!',
    'Alla prossima!',
    'Grazie {}, ma sono solo un umile barman ;)',
    'Troppo gentile!',
    'Infinite grazie {}',
    'Troppo buono :D '
]

concorde = [
    'a-ah',
    'può essere',
    'concordo',
    'davvero?',
    'incredibile!'
]

incipits = [
    'A lei {},',
    'Ecco,',
    'In arrivo all\'hangar 42',
    'Specialità della casa:',
    'Dalla riserva speciale',
    'Solo per lei Comandante {}',
    'Ecco a lei,',
    'Tutto suo,',
    'Appena scongelato, ',
    'Appena stappato, ',
    'È lì da un po\'... credo, ',
    'Non me lo chiede mai nessuno, ',
    'Direttamente dal nostro serbatoio, ',
    '(Sputa nel bicchiere) ecco, '
]

drinkings = [
    "un buon bicchiere di {}",
    "una bottiglia di {} dell'annata buona",
    "un fusto di {}",
    "ho appena aperto una bottiglia di {}",
    "un bicchiere di {} con l'ombrellino",
    "un bicchiere di {} senza l'ombrellino (li ho finiti!)"
]

conclusions = [
    ".",
    ", ci vuole anche del ghiaccio?",
    ", altro?",
    ", la metto sul conto del boss?",
    ", offre la casa!",
    ", offre la ditta!",
    ", fresco fresco (eheh).",
    ", salute!",
    ", cin-cin!"
]

notFound = [
    'Purtroppo è finito!',
    'Qui non serviamo quella robaccia...',
    'L\'ultima bottiglia è finita ieri.',
    'I trasporti sono in ritardo, ci vorrà un po\' prima che arrivi nuova merce',
    'Finito...',
    'Arriva martedì',
    'Purtroppo qui non serviamo quella robaccia...',
    'Questo è un bar di un certo livello!',
    'Mi ritengo offeso da questa richiesta!',
    'Il prossimo?',
    'Avanti un altro!',
    'Mi dispiace, è stato riservato per un\'occasione speciale.',
    'Non mi sembra in grado di reggersi in piedi...',
    'Fi-ni-to-!',
    'Esaurito...',
    'Lo devo ordinare'
]
